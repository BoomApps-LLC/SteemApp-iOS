...
static CGFloat kDefaultScale = 1.0;
...
- (void)viewDidLoad {
    ...
    if (![self isIpad]) {
        ...
        UIView *sv0 = toolbarCropper;
        
        [[keyboardToolbar.widthAnchor constraintEqualToConstant: 44.0] setActive:YES];
        [[keyboardToolbar.rightAnchor constraintEqualToAnchor: sv0.rightAnchor constant: 0.] setActive:YES];
        [[keyboardToolbar.topAnchor constraintEqualToAnchor: sv0.topAnchor constant: 0.] setActive:YES];
        [[keyboardToolbar.bottomAnchor constraintEqualToAnchor: sv0.bottomAnchor constant: 0.] setActive:YES];
        
        keyboardToolbar.translatesAutoresizingMaskIntoConstraints = NO;
        
        // Alphaed image
        UIImageView *darker = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"darker.png"]];
        darker.contentMode = UIViewContentModeScaleToFill;
        //darker.userInteractionEnabled = YES;
        [self.toolbarHolder addSubview:darker];
        
        UIView *sv11 = self.toolbarHolder;
        [[darker.rightAnchor constraintEqualToAnchor: sv11.rightAnchor constant: -44.] setActive:YES];
        [[darker.widthAnchor constraintEqualToConstant:44.] setActive:YES];
        [[darker.topAnchor constraintEqualToAnchor: sv11.topAnchor constant: 0.] setActive:YES];
        [[darker.bottomAnchor constraintEqualToAnchor: sv11.bottomAnchor constant: 0.] setActive:YES];
        
        darker.translatesAutoresizingMaskIntoConstraints = NO;
        //darker.layer.borderColor = [UIColor redColor].CGColor;
        //darker.layer.borderWidth = 2.0;
        
        // Toolbar cropper
        [self.toolbarHolder addSubview:toolbarCropper];
        
        UIView *sv1 = self.toolbarHolder;
        [[toolbarCropper.rightAnchor constraintEqualToAnchor: sv1.rightAnchor constant: 0.] setActive:YES];
        [[toolbarCropper.widthAnchor constraintEqualToConstant:44.] setActive:YES];
        [[toolbarCropper.topAnchor constraintEqualToAnchor: sv1.topAnchor constant: 0.] setActive:YES];
        [[toolbarCropper.bottomAnchor constraintEqualToAnchor: sv1.bottomAnchor constant: 0.] setActive:YES];
        
        toolbarCropper.translatesAutoresizingMaskIntoConstraints = NO;
        
        self.keyboardItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"ZSSkeyboard_open.png"] style:UIBarButtonItemStylePlain target:self action:@selector(dismissKeyboard)];
        self.keyboardItem.tintColor = [self barButtonItemDefaultColor];
        //self.keyboardButton = [[UIButton alloc] initWithFrame:CGRectZero];
        
        //[self.keyboardButton setImage:[UIImage imageNamed:@"ZSSkeyboard.png"] forState: UIControlStateNormal];
        //[self.keyboardButton addTarget:self action:@selector(dismissKeyboard) forControlEvents: UIControlEventTouchUpInside];
        ...
        UIView *sv2 = keyboardToolbar;
        [[line.widthAnchor constraintEqualToConstant: 0.6f] setActive:YES];
        [[line.leftAnchor constraintEqualToAnchor: sv2.leftAnchor constant: 0.] setActive:YES];
        [[line.topAnchor constraintEqualToAnchor: sv2.topAnchor constant: 0.] setActive:YES];
        [[line.bottomAnchor constraintEqualToAnchor: sv2.bottomAnchor constant: 0.] setActive:YES];
...
- (UIColor *)titleNormalColor {
    return [UIColor colorWithRed:63.0/255.0 green:59.0/255.0 blue:69.0/255.0 alpha:1];
}

- (UIColor *)titlePlaceholderColor {
    return [UIColor colorWithRed:172.0/255.0 green:166.0/255.0 blue:180.0/255.0 alpha:1];
}
...
- (void)createSourceView {
    ...
    self.sourceView = [[UITextView alloc] initWithFrame: CGRectZero];
...
- (void) flip: (UIView *) view to: (UIView *) superview withLeading: (CGFloat) leading
     withTrailing: (CGFloat) trailing withTop: (CGFloat) top withBottom: (CGFloat) bottom {
    UIView* sv = superview;
    
    [[view.leadingAnchor constraintEqualToAnchor: sv.leadingAnchor constant: leading] setActive:YES];
    [[view.trailingAnchor constraintEqualToAnchor: sv.trailingAnchor constant: trailing] setActive:YES];
    [[view.topAnchor constraintEqualToAnchor: sv.topAnchor constant: top] setActive:YES];
    [[view.bottomAnchor constraintEqualToAnchor: sv.bottomAnchor constant: bottom] setActive:YES];
...
- (void)createToolBarScroll {
    ...
    // Toolbar scroll
    self.toolBarScroll = [[UIScrollView alloc] initWithFrame:CGRectZero];
...
- (void) addConstraints {
    self.toolbarHolder.translatesAutoresizingMaskIntoConstraints = NO;
    
    // Toolbar
    UIView *sv1 = self.editorView.superview;
    
    [[self.toolbarHolder.leadingAnchor constraintEqualToAnchor: sv1.leadingAnchor constant: 0] setActive:YES];
    [[self.toolbarHolder.trailingAnchor constraintEqualToAnchor: sv1.trailingAnchor constant: 0] setActive:YES];
    [[self.toolbarHolder.heightAnchor constraintEqualToConstant: 44.0] setActive:YES];
    
    if (_alwaysShowToolbar) {
        [[self.toolbarHolder.bottomAnchor constraintEqualToAnchor: sv1.bottomAnchor constant: -44.] setActive:YES];
    } else {
        [[self.toolbarHolder.bottomAnchor constraintEqualToAnchor: sv1.bottomAnchor constant: 0.] setActive:YES];
    }
    
    // Editor view
    UIView *sv2 = self.editorView.superview;
    
    [[self.editorView.leftAnchor constraintEqualToAnchor: sv2.leftAnchor constant: 0] setActive:YES];
    [[self.editorView.rightAnchor constraintEqualToAnchor: sv2.rightAnchor constant: 0] setActive:YES];
    [[self.editorView.topAnchor constraintEqualToAnchor: sv2.topAnchor constant: 0] setActive:YES];
    [[self.editorView.bottomAnchor constraintEqualToAnchor: self.toolbarHolder.topAnchor constant: 0.] setActive:YES];
    
    //self.editorHeight = [self.editorView.heightAnchor constraintEqualToConstant:100];
    //[self.editorHeight setActive:YES];
    
    self.editorView.translatesAutoresizingMaskIntoConstraints = NO;
    
    // Source view
    UIView *sv3 = self.sourceView.superview;
    
    [[self.sourceView.leadingAnchor constraintEqualToAnchor: sv3.leadingAnchor constant: 0] setActive:YES];
    [[self.sourceView.trailingAnchor constraintEqualToAnchor: sv3.trailingAnchor constant: 0] setActive:YES];
    [[self.sourceView.topAnchor constraintEqualToAnchor: sv3.topAnchor constant: 0] setActive:YES];
    [[self.sourceView.bottomAnchor constraintEqualToAnchor: self.toolbarHolder.topAnchor constant: 0.] setActive:YES];
    
    self.sourceView.translatesAutoresizingMaskIntoConstraints = NO;
    
    // Toolbar scroll view
    UIView *sv4 = self.toolbarHolder;
    
    [[self.toolBarScroll.leftAnchor constraintEqualToAnchor: sv4.leftAnchor constant: 0] setActive:YES];
    [[self.toolBarScroll.topAnchor constraintEqualToAnchor: sv4.topAnchor constant: 0] setActive:YES];
    [[self.toolBarScroll.rightAnchor constraintEqualToAnchor: sv4.rightAnchor constant: -44] setActive:YES];
    [[self.toolBarScroll.bottomAnchor constraintEqualToAnchor: sv4.bottomAnchor constant: 0] setActive:YES];
    
    self.toolBarScroll.translatesAutoresizingMaskIntoConstraints = NO;
    
    
//    UIView *sv5 = self.toolBarScroll;
//
//    [[self.toolbar.leftAnchor constraintEqualToAnchor: sv5.leftAnchor constant: 0] setActive:YES];
//    [[self.toolbar.rightAnchor constraintEqualToAnchor: sv5.rightAnchor constant: 0.] setActive:YES];
//    [[self.toolbar.topAnchor constraintEqualToAnchor: sv5.topAnchor constant: 0] setActive:YES];
//    [[self.toolbar.bottomAnchor constraintEqualToAnchor: sv5.bottomAnchor constant: 0.] setActive:YES];
//
...
- (void)createParentHoldingView {
    ...
    //Background Toolbar
    UIToolbar *backgroundToolbar = [[UIToolbar alloc] initWithFrame:CGRectZero];
    ...
    [[backgroundToolbar.leftAnchor constraintEqualToAnchor: sv4.leftAnchor constant: 0] setActive:YES];
    [[backgroundToolbar.rightAnchor constraintEqualToAnchor: sv4.rightAnchor constant: 0.] setActive:YES];
    [[backgroundToolbar.topAnchor constraintEqualToAnchor: sv4.topAnchor constant: 0] setActive:YES];
    [[backgroundToolbar.bottomAnchor constraintEqualToAnchor: sv4.bottomAnchor constant: 0.] setActive:YES];

    backgroundToolbar.translatesAutoresizingMaskIntoConstraints = NO;
}
...
- (NSArray *)itemsForToolbar {
    ...
    // Blockquote
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarBlockquote]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *blockquote = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"ZSSblockquote.png" inBundle:bundle compatibleWithTraitCollection:nil] style:UIBarButtonItemStylePlain target:self action:@selector(setBlockquote)];
        blockquote.label = @"blockquote";
        if (customOrder) {
            [items replaceObjectAtIndex:[_enabledToolbarItems indexOfObject:ZSSRichTextEditorToolbarBlockquote] withObject:blockquote];
        } else {
...
- (void)buildToolbar {
    ...
    CGFloat toolbarWidth = items.count == 0 ? 0.0f : (CGFloat)((items.count + 1) * (44 + 10)) - 10;
    
    if(self.customBarButtonItems != nil)
    {
        
        items = [self.customBarButtonItems arrayByAddingObjectsFromArray:items];
...
//- (void)removeCustomToolbarItem:(ZSSBarButtonItem *)barButtonItem {
//    [self.customBarButtonItems removeObject:barButtonItem];
//    [self buildToolbar];
//}
...
- (void)showInsertImageDialogWithLink:(NSString *)url alt:(NSString *)alt {
    ...
    if ([NSProcessInfo instancesRespondToSelector:@selector(isOperatingSystemAtLeastVersion:)]) {
        ...
        [alertController addTextFieldWithConfigurationHandler:^(UITextField *textField) {
            textField.placeholder = NSLocalizedString(@"An alternate text", nil);
            textField.clearButtonMode = UITextFieldViewModeAlways;
            textField.secureTextEntry = NO;
            if (alt) {
                textField.text = alt;
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:insertButtonTitle style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            ...
            if (!self.selectedImageURL) {
                [self insertImage:imageURL.text alt:alt.text width:@"90%" height:@"auto"];
            } else {
                [self updateImage:imageURL.text alt:alt.text width:@"90%" height:@"auto"];
            }
            [self focusTextEditor];
        }]];
        [self presentViewController:alertController animated:YES completion:NULL];
        ...
        alt1.placeholder = NSLocalizedString(@"An alternate text", nil);
...
//        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Insert Image From Library", nil) message:nil preferredStyle:UIAlertControllerStyleAlert];
//        
//        //Add alt text field
//        [alertController addTextFieldWithConfigurationHandler:^(UITextField *textField) {
//            textField.placeholder = NSLocalizedString(@"An alternate text", nil);
...
//            textField.clearButtonMode = UITextFieldViewModeAlways;
//            textField.secureTextEntry = NO;
//            textField.placeholder = NSLocalizedString(@"Scale, 0.9 of screen by default", nil);
...
//        [self presentViewController:alertController animated:YES completion:NULL];
        ...
        textFieldAlt.placeholder = NSLocalizedString(@"An alternate text", nil);
        ...
        textFieldScale.placeholder = NSLocalizedString(@"Scale, 0.9 of screen by default", nil);
...
- (void)insertImage:(NSString *)url alt:(NSString *)alt width: (NSString *) width height: (NSString *) height {
    NSString *trigger = [NSString stringWithFormat:@"zss_editor.insertImage(\"%@\", \"%@\", \"%@\", \"%@\");", url, alt, width, height];
    [self.editorView stringByEvaluatingJavaScriptFromString:trigger];
}


- (void)updateImage:(NSString *)url alt:(NSString *)alt width: (NSString *) width height: (NSString *) height {
    NSString *trigger = [NSString stringWithFormat:@"zss_editor.updateImage(\"%@\", \"%@\", \"%@\", \"%@\");", url, alt, width, height];
    [self.editorView stringByEvaluatingJavaScriptFromString:trigger];
}
...
- (void)editorDidChangeWithText:(NSString *)text andHTML:(NSString *)html  {
    [self.delegate richTextEditor:self didChangeWithText:text andHTML:html];
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info{
    ...
    NSURL *path = [info valueForKey: UIImagePickerControllerImageURL];
    PHAsset *asset = [info valueForKey: UIImagePickerControllerPHAsset];
    
    CGFloat scale = self.selectedImageScale * 100;
    NSString *percent = [NSString stringWithFormat:@"%.f%%", scale];
    
    if (!self.selectedImageURL) {
        [self insertImage:path.absoluteString alt:@"" width:percent height:@"auto"];
    } else {
...
- (void)keyboardWillShowOrHide:(NSNotification *)notification {
    ...
    if ([notification.name isEqualToString: UIKeyboardWillShowNotification]) {
        ...
        //[self setFooterHeight: keyboardHeight + 44.0];
        [self setFooterHeight: 0.0];
        [self setContentHeight: self.editorView.frame.size.height - 20.0];
        
        [UIView animateWithDuration:duration delay:0 options:animationOptions animations:^{
            [self.view layoutIfNeeded];
        } completion: ^(BOOL finished) {
            self.keyboardHidden = NO;
            self.keyboardItem.image = [UIImage imageNamed:@"ZSSkeyboard.png"];
        }];
    } else {
        [UIView animateWithDuration:duration delay:0 options:animationOptions animations:^{
            [self.view layoutIfNeeded];
            ...
            self.keyboardHidden = YES;
            self.keyboardItem.image = [UIImage imageNamed:@"ZSSkeyboard_open.png"];
        }];
    }
...
- (UIColor *)barButtonItemDefaultColor {
    ...
    return [UIColor colorWithRed:63.0f/255.0f green:59.0f/255.0f blue:69.0f/255.0f alpha:1.0f];
}
...
- (UIColor *)barButtonItemSelectedDefaultColor {
    ...
    return [UIColor colorWithRed:137.0f/255.0f green:77.0f/255.0f blue:235.0f/255.0f alpha:1.0f];;
}
